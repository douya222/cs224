...
def minibatch_parse(sentences, model, batch_size):
    ...
    partial_parses = [PartialParse(s) for s in sentences] # sentence -> object or instance
    unfinished_parses = partial_parses[:] # shallow copy
    n = len(partial_parses)

    while (n > 0) :
        length = min(n,batch_size)
        transitions = model.predict(unfinished_parses[:length])
        for prase, trans in zip(unfinished_parses[:length], transitions):
            prase.parse_step(trans)
            if len(prase.stack) == 1:
                unfinished_parses.remove(prase)
                n -= 1
